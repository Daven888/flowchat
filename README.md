...
### 为什么 mock 模型不需要 API Key？

Mock Provider 在本地模拟生成回复，不调用任何外部模型服务，因此不需要 API Key。Mock 模型始终启用，适合本地开发、测试流式功能和演示。

### 为什么真实模型调用失败可能是 API Key 未设置？

真实模型需要通过在环境变量中设置 API Key 才能调用。如果未设置对应的环境变量，请求会因缺少 Authorization 头而失败。可以通过检查环境变量是否已设置来排查。

### 为什么前端不直接调用模型 API？

将所有模型调用集中在后端有以下好处：
- API Key 只在后端环境变量中，不暴露给浏览器
- 可以在后端统一做鉴权、日志、统计、敏感词过滤
- 前端只需要对接 FlowChat 后端，不关心底层模型厂商的协议差异
- 方便后续替换、新增模型服务

### 为什么中文在 PowerShell 里测试时可能出现乱码？

PowerShell 的默认字符编码可能导致中文显示异常。如果用 curl 测试，可以在 `configs/config.yaml` 中确认 MySQL 的 DSN 包含 `charset=utf8mb4`。用前端页面测试不会有此问题。

### 为什么同一个会话同时只能生成一个回复？

多轮对话依赖消息顺序。如果同一个会话中多个生成请求同时执行，可能出现上下文读取混乱、消息顺序错乱和回复不一致的问题。因此系统使用 Redis 锁限制同一会话同一时间只允许一个生成请求。
